...
const EditProduct = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const [productRes, brandRes, catRes] = await Promise.all([
                ...
                const brandMatch = (brandRes.data || []).find(
                    (b) => b.name === p.brandName
                );

                setForm({
                    name: p.name || '',
                    description: p.description || p.desc || '',
                    pictureUrl: p.pictureUrl || p.image || '',
                    price: p.price?.toString() || '',
                    brandId: brandMatch?.id?.toString() || '',
                    categoryId: p.categoryId?.toString() || '',
                    badge: p.badge || '',
                    rating: p.rating?.toString() || '',
                    reviewsCount: p.reviews?.toString() || '',
                    specsJson: p.specsJson || (p.specs ? JSON.stringify(p.specs) : ''),
                });
            } catch (err) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const msg = err.response?.data?.message || err.response?.data?.title || 'Failed to update product';
            toast.error(msg);
        } finally {
            setSubmitting(false);
        }
